...
async def send_to_all_web_clients_via_ws(data):
...
async def transcribe_audio(record_thread, transcribe_thread):
    while record_thread.is_alive() and transcribe_thread.is_alive():
        ...
        if ts and data and 'sentence' in data and data['words']:  # filtering out empty results
            ...
            await send_to_all_web_clients_via_ws({'recognition': {ts: data['sentence']}})
...
async def infer(last_minutes: int):
    ...
    if not sentences_dict:
        await send_to_all_web_clients_via_ws({'inference': {'status': 'no start'}})
        return
    ...
    try:
        await send_to_all_web_clients_via_ws({'inference': {'status': 'start'}})

        async for chunk in get_openai_response(prompt, sentences):
            if chunk:
                answer += chunk
                await asyncio.sleep(0.001)
                await send_to_all_web_clients_via_ws({'inference': {'data': chunk}})

        await send_to_all_web_clients_via_ws({'inference': {'status': 'end'}})

        if USE_TG:
            await send_to_tg(answer)
